...
#alfi
@dataclass(kw_only=True)
class LinearControlLSEMGaussianConditional(LSEMGaussianConditional):
    r"""A conditional Gaussian density, with a linear squared exponential mean (LSEM) function and a linear control.

    .. math::

        p(Y|X) = {\cal N}(\mu(X), \Sigma)

    with the conditional mean function :math:`mu(X) = M \phi(X) + B_u U + b`.
    :math:`\phi(X)` is a feature vector of the form
    ...
    Args:
        M: Matrix in the mean function.
        b: Vector in the conditional mean function.
        B_u: Matrix associated with controls.
        W: Parameters for linear mapping in the nonlinear functions.
        Sigma: The covariance matrix of the conditional.
        Lambda: Information (precision) matrix of the Gaussians.
        ln_det_Sigma: Log determinant of the covariance matrix.
    ...
    def __post_init__(self):
        ...
        dummy_B_u = jnp.array(np.random.randn(1, self.Dy, self.Du))
        dummy_output = jnp.einsum("abc,dc->adb", dummy_B_u, dummy_input)
        assert dummy_output.shape == (1, 1, self.Dy)
        self.w0 = self.W[:, 0]
        self.W = self.W[:, 1:]
        self.update_phi()
    ...
    def set_control_variable(self, u: Float[Array, "R Du"]) -> LSEMGaussianConditional:
        u = u.reshape((1, self.Du))
        R = u.shape[0]
        M = self.M
        b_u = jnp.einsum("abc,dc->db", self.B_u, u) + self.b
        tile_dims = (R, 1, 1)
        #alfi made a new edit on 13/02/2025 as defining W here as self.W didnt account for the w0 terms
        return LSEMGaussianConditional(
            M=jnp.tile(M, tile_dims),
            b=b_u,
            W=jnp.hstack((self.w0[:, None], self.W)) ,
    ...
    def affine_joint_transformation(self, p_x: pdf.GaussianPDF, u: Float[Array, "R Du"], **kwargs) -> pdf.GaussianPDF:
        cond_gauss = self.set_control_variable(u)
        return cond_gauss.affine_joint_transformation(p_x)

    def affine_marginal_transformation(self, p_x: pdf.GaussianPDF, u: Float[Array, "R Du"], **kwargs) -> pdf.GaussianPDF:
        cond_gauss = self.set_control_variable(u)
        return cond_gauss.affine_marginal_transformation(p_x)

    def affine_conditional_transformation(self, p_x: pdf.GaussianPDF, u: Float[Array, "R Du"], **kwargs) -> "ConditionalGaussianPDF":
        cond_gauss = self.set_control_variable(u)
        return cond_gauss.affine_conditional_transformation(p_x)

    def conditional_entropy(self, p_x: pdf.GaussianPDF, u: Float[Array, "R Du"], **kwargs) -> jnp.ndarray:
        p_xy = self.affine_joint_transformation(p_x, u)
        return p_xy.entropy() - p_x.entropy()

    def integrate_log_conditional(self, phi_yx: measure.GaussianMeasure, u: Float[Array, "1 Du"], **kwargs) -> jnp.ndarray:
        if u.shape[0] != 1:
            raise NotImplementedError("Only implemented for a single input.")
        cond_gauss = self.set_control_variable(u)
        return cond_gauss.integrate_log_conditional(phi_yx)

    def integrate_log_conditional_y(self, phi_x: measure.GaussianMeasure, u: Float[Array, "1 Du"], y: Float[Array, "N Dy"] = None, **kwargs) -> Union[callable, Float[Array, "N"]]:
        if u.shape[0] != 1:
            raise NotImplementedError("Only implemented for a single input.")
        cond_gauss = self.set_control_variable(u)
